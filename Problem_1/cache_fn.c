...
/* Function to simulate NINE Policy */
void NINE(char ** argv)
{
	cache **L2;
	cache **L3; 
...
/*Function to simulate Exclusive Policy */
void Exclusive(char ** argv)
{
	cache **L2;
	cache **L3; 
	L2 = init(L2B,L2R,L2A);
	...
	while (!feof(fp)) {
		...
		else
		{
			...
			if(Hit_Miss(L3,L3R,L3A,L3B,Addr,Time))
			{
				Invalidate(L3,L3B,L3R,L3A,Addr);
				ull Add_Evi_L2 = Miss_Handler(L2,L2B,L2R,L2A,Addr,Time);
				ull Add_Evi_L3 = Miss_Handler(L3,L3B,L3R,L3A,Add_Evi_L2,Time);
				L3_Hit++;
